...
function assertSignerAddressMatches(
  actual: string,
  expected: string,
  label: string,
): void {
  if (normalizeAddress(actual) !== normalizeAddress(expected)) {
    throw new Error(
      `${label} private key does not match expected signer address. expected=${expected} actual=${actual}`,
    );
  }
}
...
export async function executeSettlement(
  ...
  const [agentAOnChain, agentBOnChain, treasuryAddress] = await Promise.all([
    contractReader.agentA() as Promise<string>,
    contractReader.agentB() as Promise<string>,
    contractReader.treasury() as Promise<string>,
  ]);

  assertSignerAddressMatches(
    agentAWallet.address,
    input.expectedAgentAAddress,
    "agentA",
  );
  assertSignerAddressMatches(
    agentBWallet.address,
    input.expectedAgentBAddress,
    "agentB",
  );
  assertSignerAddressMatches(
    agentAOnChain,
  ...
  let approveTxBHash: string | null = null;
  ...
  if (mode === "standard") {
    ...
    const timeoutSeconds = Number(
      (await contractReader.AUTO_CLAIM_TIMEOUT()) as bigint,
    );
  ...
  const [treasuryBalanceAfter, released] = await Promise.all([
    ...
    contractReader.released() as Promise<boolean>,
  ]);
