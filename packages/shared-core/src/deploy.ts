...
function ensureHexAddress(value: string, label: string): string {
...
function ensureHexPrivateKey(value: string): string {
  const normalized = value.trim();
  if (!/^0x[a-fA-F0-9]{64}$/.test(normalized)) {
    throw new Error(
      "Invalid deployer private key: expected 32-byte 0x-prefixed hex.",
    );
  }
  return normalized;
}
...
async function loadCompiledEscrowArtifact(
  contractsDir: string,
...
async function deployWithFoundry(
  ...
  const agentAAddress = ensureHexAddress(
    onChain.agentAAddress,
    "agentAAddress",
  );
  const agentBAddress = ensureHexAddress(
    onChain.agentBAddress,
    "agentBAddress",
  );
  const treasuryAddress = ensureHexAddress(
    terms.treasuryAddress,
    "terms.treasuryAddress",
  );
  ...
  if (!bytecode || !bytecode.startsWith(ZERO_X_PREFIX)) {
    throw new Error(
      "Compiled DarkMatterEscrow artifact is missing deployable bytecode.",
    );
  }
  ...
  const contract = await factory.deploy(
    ...
    agentBAddress,
    treasuryAddress,
    terms.revenueShareBpsAgentA,
    terms.revenueShareBpsAgentB,
    { value: BigInt(onChain.valueWei) },
  );
  ...
  if (!receipt || !receipt.contractAddress || !receipt.hash) {
    throw new Error(
      "Deployment transaction did not produce a contract receipt.",
    );
  }

  return {
    contractAddress: ensureHexAddress(
...
export async function deployDarkMatterAgreement(
  ...
  if (!input.dryRun) {
    if (!input.onChain) {
      throw new Error(
        "onChain deployment config is required when dryRun is false.",
      );
    }
  ...
  const basePayload = {
    ...
    contractType: "DarkMatterEscrow" as const,
    deployedAt,
    dryRun: input.dryRun,
