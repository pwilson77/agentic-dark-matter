...
function buildResponse(request, status, detail, summary, onChainTxHash) {
    return {
...
async function tryTimeoutOverride(config) {
    try {
...
export function executeOperatorAction(request) {
    if (!request.poolId || !request.action) {
        return buildResponse(request, "rejected", "Operator action requires poolId and action.");
    }
    if (request.action === "retry-step") {
        return buildResponse(request, "accepted", `Queued retry for step ${request.stepId || "latest"}.`);
    }
    if (request.action === "force-reveal-public-summary") {
        return buildResponse(request, "accepted", "Queued public summary reveal with policy-safe filtering.", "Public summary reveal queued.");
    }
    if (request.action === "escalate-dispute") {
        return buildResponse(request, "accepted", `Dispute escalation marker created.${request.reason ? ` Reason: ${request.reason}` : ""}`);
    }
    return buildResponse(request, "rejected", "Unsupported operator action.");
}
export async function executeOperatorActionWithOnChain(request, onChain) {
    const response = executeOperatorAction(request);
    const allowOnChainOverride = response.status === "accepted" &&
